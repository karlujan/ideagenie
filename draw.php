...
$query_a = $database->query("SELECT Adj FROM draw ORDER BY RANDOM() LIMIT 1" );
$data_a = $query_a->fetchAll(PDO::FETCH_ASSOC);

$query_s = $database->query("SELECT Sub FROM draw ORDER BY RANDOM() LIMIT 1" );
$data_s = $query_s->fetchAll(PDO::FETCH_ASSOC);

$query_c = $database->query("SELECT Color FROM draw ORDER BY RANDOM() LIMIT 1" );
 ...
 <div class='wrapper'>
  ...
  <input type='checkbox' id='menu'>
    <ul id='nav_list'>
      ...
      <li><a href='https://atec.utdallas.app/~knl180001/ideagenie/draw.php' class='page_active'>Draw</a></li>
      <li><a href='https://atec.utdallas.app/~knl180001/ideagenie/design.php' class='page_link'>Design</a></li>
      <li><a href='https://atec.utdallas.app/~knl180001/ideagenie/about.html' class='page_link'>About</a></li>
    </ul>
  </nav>
<h1 class='draw_header'>Draw This</h1>
</div>";

foreach ($data_a as $field_a)
{
	echo "<div class='results_all1'><h2>Adjective</h2><br><p class='gen_results'>{$field_a['Adj']}</p></div>";
}

foreach ($data_s as $field_s)
{
	echo "<div class='results_all2'><h2>Subject</h2><br><p class='gen_results'> {$field_s['Sub']}</p></div>";
}

foreach ($data_c as $field_c)
{
	echo "<div class='results_all3'><h2>Color</h2><br><p class='gen_results'>{$field_c['Color']}</p></div>";
}
